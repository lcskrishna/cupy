...
errorInvalidValue = hipErrorInvalidValue
errorMemoryAllocation = hipErrorMemoryAllocation
errorPeerAccessAlreadyEnabled = hipErrorPeerAccessAlreadyEnabled
errorContextIsDestroyed = cudaErrorContextIsDestroyed
errorInvalidResourceHandle = hipErrorInvalidResourceHandle
deviceAttributeComputeCapabilityMajor = hipDeviceAttributeComputeCapabilityMajor
deviceAttributeComputeCapabilityMinor = hipDeviceAttributeComputeCapabilityMinor
...
class CUDARuntimeError(RuntimeError):
    ...
    def __init__(self, status):
        self.status = status
        cdef bytes name = hipGetErrorName(<Error>status)
        cdef bytes msg = hipGetErrorString(<Error>status)
...
cpdef int driverGetVersion() except? -1:
    cdef int version
    status = hipDriverGetVersion(&version)
    check_status(status)
    return version
...
cpdef int runtimeGetVersion() except? -1:
    ...
    ELSE:
        status = hipRuntimeGetVersion(&version)
        check_status(status)
    return version
...
cpdef int deviceGetAttribute(int attrib, int device) except? -1:
    cdef int ret
    status = hipDeviceGetAttribute(&ret, <DeviceAttr>attrib, device)
    check_status(status)
    return ret

cpdef getDeviceProperties(int device):
    cdef DeviceProp props
    cdef int status = hipGetDeviceProperties(&props, device)
    check_status(status)
...
cpdef str deviceGetPCIBusId(int device):
    ...
    status = hipDeviceGetPCIBusId(pci_bus_id, 13, device)
    check_status(status)
...
cpdef int deviceCanAccessPeer(int device, int peerDevice) except? -1:
    cdef int ret
    status = hipDeviceCanAccessPeer(&ret, device, peerDevice)
    check_status(status)
    return ret

cpdef deviceEnablePeerAccess(int peerDevice):
    status = hipDeviceEnablePeerAccess(peerDevice, 0)
    check_status(status)

cpdef deviceDisablePeerAccess(int peerDevice):
    status = hipDeviceDisablePeerAccess(peerDevice)
    check_status(status)
...
cpdef _deviceEnsurePeerAccess(int peerDevice):
    ...
    if status == 0:
        ...
        hipGetLastError()  # clear error status
        return
    check_status(status)

cpdef size_t deviceGetLimit(int limit) except? -1:
    cdef size_t value
    status = hipDeviceGetLimit(&value, <Limit>limit)
    check_status(status)
    return value
...
cpdef ipcCloseMemHandle(intptr_t devPtr):
    status = hipIpcCloseMemHandle(<void*>devPtr)
    check_status(status)

cpdef ipcGetEventHandle(intptr_t event):
    cdef IpcEventHandle handle
    status = hipIpcGetEventHandle(&handle, <driver.Event>event)
    check_status(status)
...
cpdef ipcGetMemHandle(intptr_t devPtr):
    ...
    status = hipIpcGetMemHandle(&handle, <void*>devPtr)
    check_status(status)
    # We need to do this due to a bug in Cython that
...
cpdef ipcOpenEventHandle(bytes handle):
    ...
    status = hipIpcOpenEventHandle(&event, handle_)
    check_status(status)
    return <intptr_t>event

cpdef ipcOpenMemHandle(bytes handle,
                       unsigned int flags=hipIpcMemLazyEnablePeerAccess):
    cdef void* devPtr
    cdef IpcMemHandle handle_
    handle_.reserved = handle
    status = hipIpcOpenMemHandle(&devPtr, handle_, flags)
    check_status(status)
...
cpdef intptr_t mallocManaged(
        size_t size, unsigned int flags=hipMemAttachGlobal) except? 0:
    if 0 < CUPY_HIP_VERSION < 40300000:
        raise RuntimeError('Managed memory requires ROCm 4.3+')
    cdef void* ptr
    with nogil:
        status = hipMallocManaged(&ptr, size, flags)
    check_status(status)
    return <intptr_t>ptr
...
cpdef intptr_t malloc3DArray(intptr_t descPtr, size_t width, size_t height,
    ...
    cdef Array ptr
    cdef Extent extent = make_hipExtent(width, height, depth)
    with nogil:
        status = hipMalloc3DArray(&ptr, <ChannelFormatDesc*>descPtr, extent,
                                   flags)
    check_status(status)
    return <intptr_t>ptr
...
cpdef intptr_t mallocArray(intptr_t descPtr, size_t width, size_t height,
    ...
    cdef Array ptr
    with nogil:
        status = hipMallocArray(&ptr, <ChannelFormatDesc*>descPtr, width,
                                 height, flags)
    check_status(status)
    return <intptr_t>ptr
...
cpdef intptr_t hostAlloc(size_t size, unsigned int flags) except? 0:
    cdef void* ptr
    with nogil:
        status = hipHostMalloc(&ptr, size, flags)
    check_status(status)
    return <intptr_t>ptr

cpdef hostRegister(intptr_t ptr, size_t size, unsigned int flags):
    with nogil:
        status = hipHostRegister(<void*>ptr, size, flags)
    check_status(status)

cpdef hostUnregister(intptr_t ptr):
    with nogil:
        status = hipHostUnregister(<void*>ptr)
    check_status(status)

cpdef free(intptr_t ptr):
    with nogil:
        status = hipFree(<void*>ptr)
    check_status(status)

cpdef freeHost(intptr_t ptr):
    with nogil:
        status = hipHostFree(<void*>ptr)
    check_status(status)

cpdef freeArray(intptr_t ptr):
    with nogil:
        status = hipFreeArray(<Array>ptr)
    check_status(status)
...
cpdef memcpy(intptr_t dst, intptr_t src, size_t size, int kind):
    with nogil:
        status = hipMemcpy(<void*>dst, <void*>src, size, <MemoryKind>kind)
    check_status(status)
...
cpdef memcpyPeer(intptr_t dst, int dstDevice, intptr_t src, int srcDevice,
                 size_t size):
    with nogil:
        status = hipMemcpyPeer(<void*>dst, dstDevice, <void*>src, srcDevice,
                                size)
    check_status(status)
...
cpdef memcpyPeerAsync(intptr_t dst, int dstDevice, intptr_t src, int srcDevice,
    ...
    with nogil:
        status = hipMemcpyPeerAsync(<void*>dst, dstDevice, <void*>src,
...
cpdef memcpy2D(intptr_t dst, size_t dpitch, intptr_t src, size_t spitch,
    ...
    with nogil:
        status = hipMemcpy2D(<void*>dst, dpitch, <void*>src, spitch, width,
                              height, kind)
    check_status(status)
...
cpdef memcpy2DAsync(intptr_t dst, size_t dpitch, intptr_t src, size_t spitch,
                    ...
                    intptr_t stream):
    with nogil:
        status = hipMemcpy2DAsync(<void*>dst, dpitch, <void*>src, spitch,
...
cpdef memcpy2DFromArray(intptr_t dst, size_t dpitch, intptr_t src,
    ...
    with nogil:
        status = hipMemcpy2DFromArray(<void*>dst, dpitch, <Array>src, wOffset,
...
cpdef memcpy2DFromArrayAsync(intptr_t dst, size_t dpitch, intptr_t src,
    ...
    with nogil:
        status = hipMemcpy2DFromArrayAsync(<void*>dst, dpitch, <Array>src,
...
cpdef memcpy2DToArray(intptr_t dst, size_t wOffset, size_t hOffset,
                      ...
                      int kind):
    with nogil:
        status = hipMemcpy2DToArray(<Array>dst, wOffset, hOffset, <void*>src,
...
cpdef memcpy2DToArrayAsync(intptr_t dst, size_t wOffset, size_t hOffset,
                           ...
                           size_t height, int kind, intptr_t stream):
    with nogil:
        status = hipMemcpy2DToArrayAsync(<Array>dst, wOffset, hOffset,
...
cpdef memcpy3D(intptr_t Memcpy3DParmsPtr):
    with nogil:
        status = hipMemcpy3D(<Memcpy3DParms*>Memcpy3DParmsPtr)
    check_status(status)

cpdef memcpy3DAsync(intptr_t Memcpy3DParmsPtr, intptr_t stream):
    with nogil:
        status = hipMemcpy3DAsync(<Memcpy3DParms*>Memcpy3DParmsPtr,
                                   <driver.Stream> stream)
    check_status(status)

cpdef memset(intptr_t ptr, int value, size_t size):
    with nogil:
        status = hipMemset(<void*>ptr, value, size)
    check_status(status)

cpdef memsetAsync(intptr_t ptr, int value, size_t size, intptr_t stream):
    with nogil:
        status = hipMemsetAsync(<void*>ptr, value, size,
                                 <driver.Stream> stream)
    check_status(status)
...
cpdef memPrefetchAsync(intptr_t devPtr, size_t count, int dstDevice,
    ...
    if 0 < CUPY_HIP_VERSION < 40300000:
        raise RuntimeError('Managed memory requires ROCm 4.3+')
    with nogil:
        status = hipMemPrefetchAsync(<void*>devPtr, count, dstDevice,
                                      <driver.Stream> stream)
    check_status(status)
...
cpdef intptr_t streamCreateWithFlags(unsigned int flags) except? 0:
    cdef driver.Stream stream
    status = hipStreamCreateWithFlags(&stream, flags)
    check_status(status)
    return <intptr_t>stream


cpdef streamDestroy(intptr_t stream):
    status = hipStreamDestroy(<driver.Stream>stream)
    check_status(status)


cpdef streamSynchronize(intptr_t stream):
    with nogil:
        status = hipStreamSynchronize(<driver.Stream>stream)
    check_status(status)
...
cpdef streamQuery(intptr_t stream):
    return hipStreamQuery(<driver.Stream>stream)


cpdef streamWaitEvent(intptr_t stream, intptr_t event, unsigned int flags=0):
    with nogil:
        status = hipStreamWaitEvent(<driver.Stream>stream,
...
cpdef streamBeginCapture(intptr_t stream, int mode=streamCaptureModeRelaxed):
    ...
    with nogil:
        status = hipStreamBeginCapture(<driver.Stream>stream,
...
cpdef intptr_t streamEndCapture(intptr_t stream) except? 0:
    ...
    with nogil:
        status = hipStreamEndCapture(<driver.Stream>stream, &g)
    check_status(status)
    return <intptr_t>g
...
cpdef bint streamIsCapturing(intptr_t stream) except*:
    ...
    with nogil:
        status = hipStreamIsCapturing(<driver.Stream>stream, &s)
...
cpdef intptr_t eventCreate() except? 0:
    cdef driver.Event event
    status = hipEventCreate(&event)
    check_status(status)
    return <intptr_t>event

cpdef intptr_t eventCreateWithFlags(unsigned int flags) except? 0:
    cdef driver.Event event
    status = hipEventCreateWithFlags(&event, flags)
    check_status(status)
    return <intptr_t>event


cpdef eventDestroy(intptr_t event):
    status = hipEventDestroy(<driver.Event>event)
    check_status(status)


cpdef float eventElapsedTime(intptr_t start, intptr_t end) except? 0:
    cdef float ms
    status = hipEventElapsedTime(&ms, <driver.Event>start, <driver.Event>end)
    check_status(status)
    return ms


cpdef eventQuery(intptr_t event):
    return hipEventQuery(<driver.Event>event)


cpdef eventRecord(intptr_t event, intptr_t stream):
    status = hipEventRecord(<driver.Event>event, <driver.Stream>stream)
    check_status(status)


cpdef eventSynchronize(intptr_t event):
    with nogil:
        status = hipEventSynchronize(<driver.Event>event)
    check_status(status)
...
cdef _ensure_context():
    """Ensure that hipCtx_t bound to the calling host thread exists.
...
cpdef uintmax_t createTextureObject(intptr_t ResDescPtr, intptr_t TexDescPtr):
    ...
    with nogil:
        status = hipCreateTextureObject(<TextureObject*>(&texobj),
...
cpdef destroyTextureObject(uintmax_t texObject):
    with nogil:
        status = hipDestroyTextureObject(<TextureObject>texObject)
    check_status(status)

cpdef uintmax_t createSurfaceObject(intptr_t ResDescPtr):
    cdef uintmax_t surfobj = 0
    with nogil:
        status = hipCreateSurfaceObject(<SurfaceObject*>(&surfobj),
                                         <ResourceDesc*>ResDescPtr)
    check_status(status)
    return surfobj
...
cpdef destroySurfaceObject(uintmax_t surfObject):
    with nogil:
        status = hipDestroySurfaceObject(<SurfaceObject>surfObject)
    check_status(status)

cdef ChannelFormatDesc getChannelDesc(intptr_t array):
    cdef ChannelFormatDesc desc
    with nogil:
        status = hipGetChannelDesc(&desc, <Array>array)
    check_status(status)
    return desc
...
cdef ResourceDesc getTextureObjectResourceDesc(uintmax_t obj):
    ...
    with nogil:
        status = hipGetTextureObjectResourceDesc(&desc, <TextureObject>obj)
    check_status(status)
    return desc
...
cdef TextureDesc getTextureObjectTextureDesc(uintmax_t obj):
    ...
    with nogil:
        status = hipGetTextureObjectTextureDesc(&desc, <TextureObject>obj)
    check_status(status)
    return desc
...
cdef PitchedPtr make_PitchedPtr(intptr_t d, size_t p, size_t xsz, size_t ysz):
    return make_hipPitchedPtr(<void*>d, p, xsz, ysz)
...
cpdef graphDestroy(intptr_t graph):
    with nogil:
        status = hipGraphDestroy(<Graph>graph)
    check_status(status)

cpdef graphExecDestroy(intptr_t graphExec):
    with nogil:
        status = hipGraphExecDestroy(<GraphExec>graphExec)
    check_status(status)
...
cpdef intptr_t graphInstantiate(intptr_t graph) except? 0:
    ...
    cdef GraphExec ge
    with nogil:
        status = hipGraphInstantiate(<GraphExec*>(&ge), <Graph>graph,
                                      NULL, NULL, 0)
    check_status(status)
    return <intptr_t>ge
...
cpdef graphLaunch(intptr_t graphExec, intptr_t stream):
    with nogil:
        status = hipGraphLaunch(<GraphExec>(graphExec), <driver.Stream>stream)
    check_status(status)
