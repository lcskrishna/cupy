...
ELSE:
    cdef extern from '../../cupy_rtc.h' nogil:
        const char *hiprtcGetErrorString(Result result)
        int hiprtcVersion(int *major, int *minor)
        int hiprtcCreateProgram(
            Program* prog, const char* src, const char* name, int numHeaders,
            const char** headers, const char** includeNames)
        int hiprtcDestroyProgram(Program *prog)
        int hiprtcCompileProgram(Program prog, int numOptions,
                                const char** options)
        int hiprtcGetCodeSize(Program prog, size_t *ptxSizeRet)
        int hiprtcGetCode(Program prog, char *ptx)
        int nvrtcGetCUBINSize(Program prog, size_t *cubinSizeRet)
        int nvrtcGetCUBIN(Program prog, char *cubin)
        int hiprtcGetProgramLogSize(Program prog, size_t* logSizeRet)
        int hiprtcGetProgramLog(Program prog, char* log)
        int hiprtcAddNameExpression(Program, const char*)
        int hiprtcGetLoweredName(Program, const char*, const char**)
...
class NVRTCError(RuntimeError):
    ...
    def __init__(self, status):
        self.status = status
        cdef bytes msg = hiprtcGetErrorString(<Result>status)
...
cpdef tuple getVersion():
    ...
    with nogil:
        status = hiprtcVersion(&major, &minor)
    check_status(status)
    return major, minor
...
cpdef compileProgram(intptr_t prog, options):
    ...
    with nogil:
        status = hiprtcCompileProgram(<Program>prog, option_num,
                                     option_vec_ptr)
    check_status(status)
...
cpdef bytes getPTX(intptr_t prog):
    ...
    with nogil:
        status = hiprtcGetCodeSize(<Program>prog, &ptxSizeRet)
    check_status(status)
    if ptxSizeRet == 0:
        return b''
    ...
    ptx_ptr = ptx.data()
    with nogil:
        status = hiprtcGetCode(<Program>prog, ptx_ptr)
    check_status(status)
...
cpdef unicode getProgramLog(intptr_t prog):
    ...
    with nogil:
        status = hiprtcGetProgramLogSize(<Program>prog, &logSizeRet)
    check_status(status)
    if logSizeRet == 0:
        return ''
    ...
    log_ptr = log.data()
    with nogil:
        status = hiprtcGetProgramLog(<Program>prog, log_ptr)
    check_status(status)
...
cpdef addNameExpression(intptr_t prog, str name):
    ...
    with nogil:
        status = hiprtcAddNameExpression(<Program>prog, c_name)
    check_status(status)
...
cpdef str getLoweredName(intptr_t prog, str name):
    ...
    with nogil:
        status = hiprtcGetLoweredName(<Program>prog, c_name, &mangled_name)
    check_status(status)
    b_name = mangled_name
